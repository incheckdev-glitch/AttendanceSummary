...
const SOURCE_CONFIG: Record<string, { table:string; businessKeys:string[] }> = {
  lead: { table:'leads', businessKeys:['lead_id'] },
  deal: { table:'deals', businessKeys:['deal_id'] },
  proposal: { table:'proposals', businessKeys:['proposal_id','proposal_number'] },
  agreement: { table:'agreements', businessKeys:['agreement_id','agreement_number'] },
  invoice: { table:'invoices', businessKeys:['invoice_id','invoice_number'] },
...
function normalizeCompanyFields(input: Record<string, unknown>) {
  ...
  if (candidate !== undefined && candidate !== null && String(candidate).trim()) out.company_name = String(candidate).trim();
...
async function resolveSourceRecord(db:any, source:string, payload:Record<string,unknown>) {
  ...
  const directCandidates = [
    ...
    payload[`${source}_uuid`], payload[`${source}Uuid`],
  ...
  for (const candidate of directCandidates) {
    ...
    const { data, error } = await db.from(config.table).select(selectCols).eq('id', value).limit(1).maybeSingle();
...
async function normalizeSourcePayload(db:any, source:string, payload:Record<string,unknown>) {
  ...
  const row = await resolveSourceRecord(db, source, out);
  if (!row?.id) return out;
  const uuid = String(row.id);
  out.id = uuid;
  out[`${source}_uuid`] = uuid;
  const config = SOURCE_CONFIG[source];
  const businessValue = config?.businessKeys.map(k => row[k]).find(v => String(v || '').trim());
  if (businessValue) out[`${source}_id`] = String(businessValue);
  return out;
}
...
function requiredSourceForAction(resource:string, action:string) {
  if (resource === 'leads' && ['convert','convert_to_deal'].includes(action)) return 'lead';
  if (resource === 'proposals' && action === 'create_from_deal') return 'deal';
  if (resource === 'agreements' && action === 'create_from_proposal') return 'proposal';
  if (resource === 'invoices' && action === 'create_from_agreement') return 'agreement';
  if (resource === 'receipts' && action === 'create_from_invoice') return 'invoice';
  return '';
}
...
async function ensureSession(db:any,sessionId:string,user:any) {
  ...
  if (data?.id) { await db.from('ai_chat_sessions').update({updated_at:new Date().toISOString()}).eq('id',sessionId); return; }
...
async function history(db:any,sessionId:string) {
  ...
  const out:any[]=[];
  for (const row of rows) { try { const text=await decrypt(row); if (text && (row.role==='user'||row.role==='assistant')) out.push({role:row.role,content:text}); } catch {} }
  return out;
}
...
async function searchRecords(db:any,args:any) {
  const resource=normalizeResource(args?.resource); const tables=matchTable(resource);
  const query=String(args?.query||'').toLowerCase().trim(); const reference=String(args?.reference||'').toLowerCase().trim(); const status=String(args?.status||'').toLowerCase().trim();
  const requested=Math.max(1,Math.min(MAX_RESULTS,Number(args?.limit||20))); const results:any[]=[]; const warnings:string[]=[];
  for (const table of tables) {
    ...
    for (const row of data||[]) {
      const hay=JSON.stringify(row).toLowerCase(); if(query&&!hay.includes(query))continue;if(reference&&!hay.includes(reference))continue;if(status&&!hay.includes(status))continue;
      results.push(compactRow(table,row)); if(results.length>=requested)return {results,warnings};
    }
  }
...
- One tool call = one resource/action. Multi-step workflows are allowed but proceed sequentially after actual results return.
...
const TOOLS:any[]=[
  {type:'function',name:'search_erp_records',strict:true,description:'Search live ERP records by module, free text, business reference, or status.',parameters:{type:'object',additionalProperties:false,properties:{resource:{type:['string','null']},query:{type:['string','null']},reference:{type:['string','null']},status:{type:['string','null']},limit:{type:['number','null']}},required:['resource','query','reference','status','limit']}},
...
Deno.serve(async(req:Request)=>{
  ...
  try{
    const url=Deno.env.get('SUPABASE_URL')||''; const key=adminKey();
    ...
    const db=createClient(url,key,{auth:{persistSession:false,autoRefreshToken:false}}); const user=await resolveAdmin(req,db);
    const body=await req.json().catch(()=>({})); const message=String(body?.message||'').trim();
    if(!message)return respond({error:'Message is required.'},400); if(message.length>MAX_MESSAGE_CHARS)return respond({error:`Message exceeds ${MAX_MESSAGE_CHARS} characters.`},400);
    const sessionId=String(body?.session_id||crypto.randomUUID()); await ensureSession(db,sessionId,user);
    const planId=String(body?.plan_id||crypto.randomUUID()); const actionResults=Array.isArray(body?.action_results)?body.action_results:null;
    if(actionResults){
      for(const r of actionResults){if(!r?.action_id)continue;await db.from('ai_action_audit').update({status:String(r.status||'unknown'),result_json:r,completed_at:new Date().toISOString()}).eq('plan_id',planId).eq('action_id',String(r.action_id)).eq('user_id',user.id);}
    }else await saveMessage(db,sessionId,'user',message,user);

    const previous=await history(db,sessionId);
    const continuation=actionResults?`Continue the original request using these actual ERP action results. Do not repeat an identical failed, blocked, successful, or cancelled action. If more work is required, inspect updated state and propose the next action. If complete, summarize the real outcome.\n\nOriginal request: ${message}\n\nAction results: ${JSON.stringify(actionResults).slice(0,12000)}`:message;
    ...
    for(let step=0;step<6;step++){
      ...
      for(const item of response?.output||[]){
        ...
        if(item.name==='execute_erp_action'){
          const resource=String(args.resource||'').trim().toLowerCase(); const action=String(args.action||'').trim().toLowerCase(); const allowed=WRITE_ACTIONS[resource]||[]; const rawPayload=jsonObject(args.payload_json);
          ...
          const actionId=crypto.randomUUID(); const planned={action_id:actionId,resource,action,payload_json:JSON.stringify(payload),summary:String(args.summary||`${action} ${resource}`),reason:String(args.reason||''),risk:mustConfirm?'high':String(args.risk||'medium'),requires_confirmation:mustConfirm};
          const {error}=await db.from('ai_action_audit').insert({plan_id:planId,action_id:actionId,session_id:sessionId,user_id:user.id,user_email:user.email,user_role:'admin',resource,action,payload_json:payload,summary:planned.summary,risk:planned.risk,requires_confirmation:planned.requires_confirmation,status:'planned'});
          if(error)throw new Error(`Unable to audit AI action: ${error.message}`); pending.push(planned);
        }
      }
      ...
      if(outputs.length){response=await openai({model:MODEL,previous_response_id:response.id,input:outputs,tools:TOOLS,parallel_tool_calls:false});continue;}
      break;
    }
    const answer=outputText(response)||'I could not complete that request with the available ERP tools.'; await saveMessage(db,sessionId,'assistant',answer,user);
