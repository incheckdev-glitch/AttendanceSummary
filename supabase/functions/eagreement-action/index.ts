  ...
  try {
    ...
    if (action === "accept" && signatureType === "signed_document_upload" && signedDocumentDataUrl) {
      const { data: record, error: recordError } = await supabase
        .from("agreements")
        .select("id,agreement_id,agreement_number")
        .eq("e_agreement_token", token)
        .maybeSingle();
      ...
      const storagePath = buildSignedDocumentStoragePath({ module: "agreements", recordId: record.id, businessNo, fileName: signedDocumentFileName });
      const blob = dataUrlToBlob(signedDocumentDataUrl, signedDocumentMimeType || "application/octet-stream");
      const { error: uploadError } = await supabase.storage
        .from("agreement-signed-documents")
      ...
      const { data: publicUrlData } = supabase.storage.from("agreement-signed-documents").getPublicUrl(storagePath);
      uploadedSignedDocumentUrl = publicUrlData?.publicUrl || storagePath;
      console.info("[SignedDocumentUpload] saved", { module: "agreements", recordId: record.id, storagePath, fileName: signedDocumentFileName });
    }
