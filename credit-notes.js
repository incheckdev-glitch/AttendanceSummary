const CreditNotes = {
  ...
  canCreate() { return !window.Permissions || Permissions.canCreateCreditNote?.() || Permissions.canCreateReceipt?.(); },
  canView() { return !window.Permissions || Permissions.canPreviewCreditNote?.() || Permissions.canViewReceipts?.(); },
  ...
  render() {
    ...
    E.creditNotesTbody.innerHTML = rows.length ? rows.map(row => {
      ...
      return `<tr>
        ...
        <td><button class="btn ghost sm" data-credit-note-preview="${id}">View / Print</button>${canCancel ? ` <button class="btn ghost sm" data-credit-note-cancel="${id}">Cancel</button>` : ''}</td>
      </tr>`;
  ...
  async preview(id) {
    if (!this.canView()) return UI.toast('You do not have permission to view credit notes.');
  ...
  exportPreviewPdf() { const frame = E.creditNotePreviewFrame; if (!frame?.contentWindow) return; frame.contentWindow.focus(); frame.contentWindow.print(); },
  bind() {
