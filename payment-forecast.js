const PaymentForecast = {
  ...
  renderPaymentForecastDetailsLogs(logs = []) {
    ...
    const body = `<div class="payment-forecast-log-timeline">${logs.map(item => `<div class="payment-forecast-log-item"><strong>${U.escapeHtml(this.date(item.created_at) || '—')}</strong><span>${U.escapeHtml(this.label(item.action_type || item.action))}</span>${item.old_status || item.new_status ? `<span>${U.escapeHtml(this.label(item.old_status))} → ${U.escapeHtml(this.label(item.new_status))}</span>` : ''}<div>${U.escapeHtml(item.note || '—')}</div><small class="muted">${U.escapeHtml(item.created_by_email || item.created_by_name || item.created_by || 'System')}</small></div>`).join('')}</div>`;
