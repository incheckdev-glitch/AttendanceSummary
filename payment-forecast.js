const PaymentForecast = {
  ...
  state: {
    ...
    followups: [], activityRow: null, activityLogs: [], activityLoading: false, activityError: '', summaryError: '', rowsError: ''
  },
  ...
  async addFollowupNote(id) { if (!this.canView()) return UI.toast('You do not have permission to add follow-up notes.'); const row = (this.state.rowsByTab[this.canonicalTab()] || []).find(item => item.forecast_row_id === id); if (!row) return; const note = window.prompt('Add a collection follow-up note:'); if (note === null) return; if (!this.text(note)) return UI.toast('A note is required.'); await Api.addPaymentForecastFollowupNote(this.followupPayload(row, { follow_up_notes: note, log_note: note })); UI.toast('Follow-up note added.'); await this.refresh(true); },
  async editFollowup(id) { if (!this.canManage()) return UI.toast('You do not have permission to manage follow-ups.'); const row = (this.state.rowsByTab[this.canonicalTab()] || []).find(item => item.forecast_row_id === id); if (!row) return; const status = window.prompt(`Follow-up status (${this.followUpStatuses.join(', ')}):`, row.follow_up_status || 'contacted'); if (status === null) return; const nextAt = window.prompt('Next follow-up date (YYYY-MM-DD, optional):', this.date(row.next_follow_up_at)); if (nextAt === null) return; const customNote = window.prompt('Optional note for this update:', ''); if (customNote === null) return; const normalized = this.followUpStatuses.includes(status.trim().toLowerCase()) ? status.trim().toLowerCase() : 'contacted'; await Api.savePaymentForecastFollowup(this.followupPayload(row, { follow_up_status: normalized, next_follow_up_at: nextAt ? `${nextAt}T09:00:00Z` : null })); if (this.text(customNote)) await Api.addPaymentForecastFollowupNote(this.followupPayload(row, { follow_up_notes: customNote, log_note: customNote })); UI.toast('Collection follow-up updated.'); await this.refresh(true); },
  async markFollowedUp(id) { if (!this.canManage()) return UI.toast('You do not have permission to manage follow-ups.'); const row = (this.state.rowsByTab[this.canonicalTab()] || []).find(item => item.forecast_row_id === id); if (!row) return; await Api.markPaymentForecastFollowedUp(this.followupPayload(row, { last_follow_up_at: new Date().toISOString() })); UI.toast('Marked as followed up.'); await this.refresh(true); },
  ...
  renderActivityModal() { const modal = document.getElementById('paymentForecastActivityModal'), row = this.state.activityRow, content = document.getElementById('paymentForecastActivityContent'); if (!modal || !content || !row) return; document.getElementById('paymentForecastActivityClient').textContent = row.client_name || '—'; document.getElementById('paymentForecastActivityInvoice').textContent = row.invoice_number || '—'; const status = document.getElementById('paymentForecastActivityStatus'); status.textContent = this.label(row.follow_up_status); status.className = this.statusClass(row.follow_up_status); if (this.state.activityLoading) { content.innerHTML = '<div class="pf-activity-state muted">Loading activity…</div>'; return; } if (this.state.activityError) { content.innerHTML = `<div class="pf-activity-state pf-error">${U.escapeHtml(this.state.activityError)}</div>`; return; } const logs = this.state.activityLogs || []; content.innerHTML = logs.length ? `<div class="table-scroll"><table class="pf-activity-table"><thead><tr><th>Date/time</th><th>Action Type</th><th>Old Status</th><th>New Status</th><th>Note</th><th>Created By</th></tr></thead><tbody>${logs.map(log => `<tr><td>${U.escapeHtml(this.formatDateTime(log.created_at || log.logged_at || log.action_at))}</td><td><strong>${U.escapeHtml(this.label(log.action_type))}</strong></td><td>${U.escapeHtml(this.label(log.old_status || '') || '—')}</td><td>${U.escapeHtml(this.label(log.new_status || '') || '—')}</td><td class="pf-notes-cell">${U.escapeHtml(log.note || '—')}</td><td>${U.escapeHtml(log.created_by_name || log.created_by_email || log.created_by || '—')}</td></tr>`).join('')}</tbody></table></div>` : '<div class="pf-activity-state muted">No activity has been recorded for this follow-up yet.</div>'; },
  async openActivity(id) { if (!this.canView()) return UI.toast('You do not have permission to view follow-up activity.'); const row = (this.state.rowsByTab[this.canonicalTab()] || []).find(item => item.forecast_row_id === id); if (!row) return; this.state.activityRow = row; this.state.activityLogs = []; this.state.activityError = ''; this.state.activityLoading = true; const modal = document.getElementById('paymentForecastActivityModal'); if (modal) { modal.hidden = false; document.body.classList.add('pf-modal-open'); } this.renderActivityModal(); if (!row.followup_id) { this.state.activityLoading = false; this.renderActivityModal(); return; } try { this.state.activityLogs = (await Api.getPaymentForecastFollowupLogs(row.followup_id)).map(item => item?.row_data || item).filter(Boolean); } catch (error) { this.state.activityError = error.message || 'Unable to load follow-up activity.'; } finally { this.state.activityLoading = false; this.renderActivityModal(); } },
  closeActivity() { const modal = document.getElementById('paymentForecastActivityModal'); if (modal) modal.hidden = true; document.body.classList.remove('pf-modal-open'); this.state.activityRow = null; this.state.activityLogs = []; this.state.activityError = ''; },
