const PaymentForecast = {
  ...
  async savePaymentForecastFollowupNote(row, note) { const cleanNote = this.text(note); if (!cleanNote) throw new Error('A note is required.'); if (!row?.followup_id) throw new Error('Follow-up ID is required to save a note.'); await Api.createPaymentForecastFollowupLog(this.followupLogPayload(row, { action_type: 'note', note: cleanNote })); if (this.state.activityRow?.followup_id === row.followup_id) await this.loadPaymentForecastFollowupLogs(row.followup_id); },
  ...
  renderPaymentForecastFollowupLogs(logs = []) { if (!logs.length) return '<div class="pf-activity-state muted">No activity logs yet.</div>'; return `<div class="pf-activity-timeline">${logs.map(log => { const creator = log.created_by_name || log.created_by_email || log.created_by || 'Unknown user'; const action = this.label(log.action_type || 'activity'); const statusChange = log.old_status || log.new_status ? `<div class="pf-activity-entry-status">${U.escapeHtml(log.old_status || '—')} → ${U.escapeHtml(log.new_status || '—')}</div>` : ''; const note = this.text(log.note) ? `<p class="pf-activity-entry-note">${U.escapeHtml(log.note)}</p>` : ''; return `<article class="pf-activity-entry"><div class="pf-activity-entry-time">${U.escapeHtml(this.formatDateTime(log.created_at || log.logged_at || log.action_at))}</div><div class="pf-activity-entry-title"><span>${U.escapeHtml(action)}</span><span class="muted">by ${U.escapeHtml(creator)}</span></div>${statusChange}${note}</article>`; }).join('')}</div>`; },
  ...
  bind() {
    ...
    document.addEventListener('keydown', event => { if (event.key !== 'Escape') return; if (document.getElementById('paymentForecastAddNoteModal')?.hidden === false) this.closePaymentForecastAddFollowupNote(); else if (document.getElementById('paymentForecastActivityModal')?.hidden === false) this.closeActivity(); });
