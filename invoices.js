const Invoices = {
  ...
  async loadInvoicePreviewData(invoiceRef) {
    ...
    const [byId, byNumber, creditById, creditByNumber, scheduleResult] = await Promise.all([
      ...
      creditQuery({ invoice_id: invoiceUuid }).catch(error => ({ data: [], error })),
      invoiceNumber ? creditQuery({ invoice_number: invoiceNumber }).catch(error => ({ data: [], error })) : Promise.resolve({ data: [], error: null }),
  ...
  renderInvoiceFooterNote() {
    ...
    return `<footer class="footer-note document-footer-note"><div>${U.escapeHtml(autoGeneratedNote)}</div><div>${U.escapeHtml(contactNote)}</div></footer>`;
  },
  buildInvoicePreviewHtml(invoice = {}, items = [], receipts = [], paymentScheduleRows = [], creditNotes = []) {
...
<html>
  ...
  async previewInvoice(invoiceId) {
    ...
    try {
      const { invoiceUuid, invoice, items, receipts, paymentSchedule, creditNotes } = await this.loadInvoicePreviewData(id);
      const html = this.buildInvoicePreviewHtml(invoice, items, receipts, paymentSchedule, creditNotes);
