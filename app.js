...
const Analytics = {
  ...
  _render(list){
    ...
    E.aiPatternsList.innerHTML = topTerms.length? topTerms.map(([t,c])=>`<li><strong>${U.escapeHtml(t)}</strong> – ${c}</li>`).join('') : '<li>No strong repeated terms recently.</li>';
    ...
    E.aiLabelsList.innerHTML = topCats.length? topCats.map(([l,n])=>`<li><strong>${U.escapeHtml(l)}</strong> – ${n}</li>`).join('') : '<li>No clear category suggestions yet.</li>';
    ...
    E.aiTrendsList.innerHTML = trend.length? trend.slice(0,8).map(o=>
      `<li><strong>${U.escapeHtml(o.t)}</strong> – ${o.new} vs ${o.old} <span class="muted">(Δ ${o.delta>=0?`+${o.delta}`:o.delta})</span></li>`
    ...
    E.aiEmergingStable.innerHTML=`
      <li><strong>Emerging:</strong> ${emerg.length?emerg.join(', '):'—'}</li>
      <li><strong>Stable:</strong> ${stable.length?stable.join(', '):'—'}</li>
    `;
    ...
    `).join(''):'<div class="muted">No similar issue groups ≥2.</div>';
