...
const Companies = {
  ...
  async openForm(existing = null) {
    ...
    const modal = document.getElementById('companyModal'); modal.style.display = 'flex'; modal.setAttribute('aria-hidden', 'false'); window.ModalScrollLock?.lock?.() || document.body.classList.add('modal-open');
  },
  closeForm() { const form = document.getElementById('companyForm'); form?.reset(); document.getElementById('companyRecordId').value = ''; document.getElementById('companyStatusInput').value = 'Prospect'; this.state.currentCompany = null; this.state.documents = []; this.renderCompanyDocumentsSection(null); this.renderCompanyVerificationPanel(null); const modal = document.getElementById('companyModal'); modal.style.display = 'none'; modal.setAttribute('aria-hidden', 'true'); window.ModalScrollLock?.unlock?.() || document.body.classList.remove('modal-open'); this.toggleSave(false); },
