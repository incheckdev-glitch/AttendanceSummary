const Companies = {
  ...
  async onAction(a, id) { const row = this.state.rows.find(x => x.id === id); if (!row) return; if (a === 'edit') this.openForm(row); if (a === 'del') { if (!confirm('Delete company?')) return; try { await Api.requestWithSession('companies', 'delete', { id }, { requireAuth: true }); await this.loadAndRefresh(); } catch (e) { UI?.toast?.('Unable to delete company', 'error'); console.error(e); } } if (a === 'contacts') { window.Contacts?.openCreateForCompany?.(row); window.App?.showView?.('contacts'); } if (a === 'lead') { window.Leads?.openCreatePrefilled?.({ company_id: row.company_id, company_name: row.company_name }); }
  },
