-- CS360 LOCATION NAME CORRECTION V2
-- Run once in Supabase SQL Editor after deploying the updated client-success.js/index.html.
--
-- Corrects the actual CS360 values seen in the completion table:
--   LR muroo / LR Muroor -> LR Muroor
--   LR Defence or ZL Defence -> LR Motor City
--   ZL Khalidya / ZL Khalidiya -> ZL al Forsan Cloud Kitchen
--
-- Only Customer Success owned tables are updated. Invoice, agreement, CRM,
-- accounting, and other source modules are not changed.

begin;

-- Brand-location snapshots: remove a duplicate old-name row only when the
-- corrected row already exists for the same brand/company, then rename.
do $$
declare
  v_old text;
  v_new text;
...
-- Saved completion rows: prevent a period/location unique conflict before
...
-- Standalone Special CS Client locations. Preserve any existing corrected
-- duplicate rather than violating the special-client location uniqueness rule.
do $$
declare
  v_old text;
  v_new text;
...
-- Any additional CS360-owned location snapshot table.
do $$
declare
  v_table record;
  v_old text;
