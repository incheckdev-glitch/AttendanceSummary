-- Keep the public e-proposal RPC signatures aligned with the eproposal-action Edge Function.
-- The Edge Function sends p_ip_address only after verifying the deployed RPC supports it;
-- this migration adds that support to the latest acceptance signature.
...
begin
  ...
  set accepted_at = now(),
      ...
      e_signature_ip_address = nullif(btrim(coalesce(p_ip_address, '')), ''),
