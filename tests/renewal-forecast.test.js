...
assert(permissions.includes("if (key === 'renewalForecast') return isMonthlyRenewalForecastAdmin"), 'renewal forecast tab access must use the shared exact-admin helper');
...
assert(frontend.includes('if (!this.isAdmin()) { this.renderAccessDenied(); return; }'), 'component rendering must show access denied and stop for non-admin users');
